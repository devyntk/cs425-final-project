        ...
        match msg {
            ...
            LoginMessage::Submit => {
                ...
                let user = client.query_opt(
                    "SELECT * FROM user_tbl WHERE username = $1 AND psswrd = $2;",
                    &[&self.username, &self.password],
                );
                match user {
                    Ok(result) => match result {
                        None => self.err_text = "No Users Found".parse().unwrap(),
                        Some(row) => {
                            self.disabled = false;
                    ...
                    Err(err) => self.err_text = err.to_string(),
                }
                self.disabled = false;
            }
        }
    ...
    pub(crate) fn view(&mut self) -> Element<Message> {
        Column::new()
            ...
            .push(text_input::TextInput::new(
                ...
                "username",
                &*self.username,
                make_wrapper(LoginMessage::UpdateUsername),
            ))
            .push(text_input::TextInput::new(
                &mut self.password_state,
                "password",
                &*self.password,
                make_wrapper(LoginMessage::UpdatePassword),
            ))
            .push(
                button::Button::new(
                    &mut self.login_button,
                    Text::new(match self.disabled {
                        true => "Logging in...",
                        false => "Log In",
                    }),
                )
