...
fn make_dep_wrapper(
    dep_id: i32,
    variant: impl Fn(i32, String) -> EmployeeMessage,
...
impl UserEmp {
    fn view(&mut self) -> Column<Message> {
        Column::new()
            .push(Row::new().push(Text::new("Username:")).push(TextInput::new(
                &mut self.username_state,
                "username",
                &*self.username,
                make_wrapper(EmployeeMessage::ChangeUsername),
            )))
            .push(Row::new().push(Text::new("Password:")).push(TextInput::new(
...
impl Dependent {
    fn view(&mut self) -> Column<Message> {
        Column::new()
            .push(Row::new().push(Text::new("D_ID:")).push(TextInput::new(
                &mut self.d_id_state,
                "D_ID",
                &*self.d_id.to_string(),
            ...
            .push(Row::new().push(Text::new("Name:")).push(TextInput::new(
                &mut self.d_name_state,
                "Name",
                &*self.d_name,
                make_dep_wrapper(self.d_id, EmployeeMessage::ChangeDepName),
            )))
            .push(Row::new().push(Text::new("SSN:")).push(TextInput::new(
                &mut self.ssn_state,
                "SSN",
                &*self.ssn,
                make_dep_wrapper(self.d_id, EmployeeMessage::ChangeDepSSN),
            )))
            .push(Row::new().push(Text::new("Relation:")).push(TextInput::new(
                &mut self.relation_state,
                "Relation",
                &*self.relation,
                make_dep_wrapper(self.d_id, EmployeeMessage::ChangeDepRelation),
            )))
            .push(Row::new().push(Text::new("Benefit:")).push(TextInput::new(
                &mut self.benefits_state,
                "Benefit",
                &*self.benefits,
                make_dep_wrapper(self.d_id, EmployeeMessage::ChangeDepBenefit),
            )))
            .push(
                Button::new(&mut self.delete_state, Text::new("Delete Dependent")).on_press(
                    Message::EmployeeMessage(EmployeeMessage::RemoveDep(self.d_id)),
                ),
            )
    }
}
...
impl EmployeeState {
    ...
    pub(crate) fn update(
        ...
        match msg {
            ...
            EmployeeMessage::ChangeJobTitle(str) => match user.usertype {
            ...
            EmployeeMessage::LoadEmployee(e_id) => {
                ...
                let years = client
                    .query("SELECT e_year FROM employeeYear WHERE e_id = $1;", &[&e_id])
                ...
                let dependents = client
                    .query("SELECT * FROM dependent WHERE e_id = $1;", &[&e_id])
                ...
                let user = client
                    .query_opt("SELECT * FROM user_tbl WHERE e_id = $1;", &[&self.e_id])
            ...
            EmployeeMessage::CreateEmployee => {
                ...
                return self.update(
                    EmployeeMessage::LoadEmployee(e.unwrap().get("e_id")),
                    client,
                    user,
                );
            }
            EmployeeMessage::SaveChanges => {
                ...
                for dep in self.dependents.values() {
                    client
                        .execute(
                            "INSERT INTO dependent (D_ID, E_ID, d_name, SSN, relation, benefits) \
                    VALUES ($1, $2, $3, $4, $5, $6) \
                ...
                if let Some(user) = &self.user_emp {
                    client
                        .execute(
                            "INSERT INTO user_tbl (username, psswrd, user_type, E_ID) \
                    VALUES ($1, $2, $3, $4) \
                ...
                return Some(Message::EmployeeMessage(EmployeeMessage::LoadEmployee(
                    self.e_id,
                )));
            }
            EmployeeMessage::LoadYear(year) => {
                return Some(Message::EmployeeYearMessage(
                    crate::employee_year::EmployeeYearMessage::Load {
                        year,
                        e_id: self.e_id,
                    },
                ))
            ...
            EmployeeMessage::RemoveDep(idx) => match self.dependents.get(&idx) {
                None => {
                    panic!("Trying to delete unknown idx")
                }
                Some(_dep) => {
                    match client.execute(
                        "DELETE FROM dependent WHERE e_id=$1 AND d_id=$2;",
                        &[&self.e_id, &idx],
                    ) {
                        Ok(_) => {}
                        Err(err) => {
            ...
            EmployeeMessage::ChangeDepName(idx, str) => match self.dependents.get_mut(&idx) {
                None => {
                    panic!("Cannot find Dep IDX")
                }
                Some(dep) => {
                    dep.d_name = str;
                }
            },
            EmployeeMessage::ChangeDepBenefit(idx, str) => match self.dependents.get_mut(&idx) {
                None => {
                    panic!("Cannot find Dep IDX")
                }
                Some(dep) => {
            ...
            EmployeeMessage::ChangeDepRelation(idx, str) => match self.dependents.get_mut(&idx) {
                None => {
                    panic!("Cannot find Dep IDX")
                }
                Some(dep) => {
            ...
            EmployeeMessage::ChangeDepSSN(idx, str) => match self.dependents.get_mut(&idx) {
                None => {
                    panic!("Cannot find Dep IDX")
                }
                Some(dep) => {
            ...
            EmployeeMessage::ChangeUsername(str) => match &mut self.user_emp {
                None => {
                    panic!("No user on record")
                }
                Some(user) => user.username = str,
            },
            EmployeeMessage::ChangePassword(str) => match &mut self.user_emp {
                None => {
                    panic!("No user on record")
                }
    ...
    pub(crate) fn view(&mut self, user: &User) -> Element<Message> {
        let column: Column<Message> = Column::new()
            ...
            .push(Row::new().push(Text::new("Address:")).push(TextInput::new(
            ...
            .push(match !self.years.is_empty() {
                true => {
                    let mut year_row: Row<Message> = Row::new().push(Text::new("Years on Record:"));

                    for (i, state) in self.year_states.iter_mut().enumerate() {
                        year_row = year_row.push(
                            Button::new(state, Text::new(self.years[i].to_string()))
            ...
            .push(
                Row::new().push(Text::new("Dependents:")).push(
                    self.dependents
                        .iter_mut()
                        .fold(Column::new(), |parent: Column<Message>, (_d_id, dep)| {
                            parent.push(dep.view())
                        })
                        .push(
                            Button::new(&mut self.add_dep_state, Text::new("Add Dependent"))
            ...
            .push(match &mut self.user_emp {
                None => Column::new().push(
                    Button::new(&mut self.make_login_state, Text::new("Make Login"))
                        .on_press(Message::EmployeeMessage(EmployeeMessage::MakeLogin)),
                ),
                Some(login_emp) => login_emp.view(),
            })
            .push(
            ...
            .push(match user.usertype {
                UserType::Employee => Button::new(&mut self.logout_button, Text::new("Log Out"))
                    .on_press(Message::LogOut),
                _ => Button::new(&mut self.logout_button, Text::new("Back to Menu")).on_press(
                    Message::EmployeeListMessage(crate::employee_list::EmployeeListMessage::Load),
                ),
            });
