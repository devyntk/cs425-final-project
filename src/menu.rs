...
use iced::{text_input, Button, Column, Element, Row, Text};
use postgres::Client;
...
impl MenuState {
    ...
    pub(crate) fn view(&mut self, user: &User) -> Element<Message> {
        Column::new()
            ...
            .push(
                Button::new(&mut self.log_out_state, Text::new("Log out"))
                    .on_press(Message::MenuMessage(MenuMessage::LogOut)),
            )
            .push(
                Button::new(&mut self.list_state, Text::new("View All Employees"))
                    .on_press(Message::MenuMessage(MenuMessage::EmployeeList)),
            )
            .push(
                Button::new(&mut self.add_employee_state, Text::new("Add New Employee")).on_press(
                    Message::EmployeeMessage(crate::employee::EmployeeMessage::CreateEmployee),
                ),
            )
            .push(match user.usertype {
                UserType::Administrator => Row::new()
                    ...
                    .push(
                        Button::new(&mut self.expense_state, Text::new("View Report")).on_press(
                            Message::EmployeeExpenseMessage(EmployeeExpenseMessage::Load(
                                self.expense_year,
                            )),
                        ),
                    ),
