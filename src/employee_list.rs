    ...
    fn view(&mut self) -> Row<Message> {
        Row::new()
            .push(Text::new(format!(
                "{} {} ({})",
                self.first_name, self.last_name, self.e_id
            )))
            .push(
                Button::new(&mut self.edit_state, Text::new("Edit/View")).on_press(
                    Message::EmployeeListMessage(EmployeeListMessage::Edit(self.e_id)),
                ),
            )
    }
}
...
impl EmployeeListState {
    ...
    pub(crate) fn update(
        ...
        match msg {
            ...
            EmployeeListMessage::Back => return Some(Message::SelectPage(Page::Main)),
            EmployeeListMessage::Edit(idx) => {
                return Some(Message::EmployeeMessage(
                    crate::employee::EmployeeMessage::LoadEmployee(idx),
                ))
            }
            EmployeeListMessage::AddEmployee => {
                return Some(Message::EmployeeMessage(
                    crate::employee::EmployeeMessage::CreateEmployee,
                ))
            }
        }
    }
    ...
    pub(crate) fn view(&mut self, _user: &User) -> Element<Message> {
        Column::new()
            ...
            .push(
                Button::new(&mut self.back_state, Text::new("Back"))
                    .on_press(Message::EmployeeListMessage(EmployeeListMessage::Back)),
            )
            .push(
                Button::new(&mut self.add_state, Text::new("Add Employee")).on_press(
                    Message::EmployeeListMessage(EmployeeListMessage::AddEmployee),
                ),
            )
            .into()
    }
